...
| 需求ID | 需求描述 | 优先级 | 涉及模块 | Beta版状态 |
|--------|---------|--------|----------|------------|
| FR-01 | 支持批量上传多个剧本文件（.txt/.md/.markdown） | P0 | 前端+后端 | ✅ 包含 |
| FR-02 | 自动识别集数编号（从文件名提取） | P1 | 后端 | ✅ 包含 |
| FR-03 | 文件内容去重检测（基于Hash） | P2 | 后端 | ⏳ V1.1后补充 |
| FR-04 | 剧本文件→结构化JSON转换 | P0 | Python微服务 | ✅ 包含 |
| FR-05 | 单文件内部逻辑检查（5类错误） | P0 | TypeScript Agent | ✅ 包含 |
| FR-06 | 跨文件一致性检查（4类新错误） | P0 | TypeScript Agent + AI | ✅ 包含 |
| FR-07 | 统一诊断报告（区分单文件/跨文件问题） | P0 | 前端+后端 | ✅ 包含 |
| FR-08 | 文件列表管理（增删改查、排序） | P1 | 前端+后端 | ✅ 包含 |
| FR-09 | 转换进度实时展示 | P2 | 前端 | ⏳ V1.1后补充 |
| FR-10 | 跨文件问题关联高亮 | P2 | 前端 | ⏳ V1.1后补充 |

**Beta版范围说明**：
- ✅ **P0功能全部包含**：核心价值链完整（多文件上传、JSON转换、跨文件检查、诊断报告）
- ✅ **P1功能全部包含**：重要体验功能保留
- ⏳ **P2功能暂缓实施**：UI增强功能延后到V1.1（节省2天开发时间）
  - FR-03（去重检测）：数据库字段保留，后期1小时可补回前端提示
  - FR-09（进度条）：用简单"转换中...N个文件"替代实时进度
  - FR-10（关联高亮）：用清晰的文本描述+颜色编码替代复杂高亮
...
### Sprint 1: 多文件基础架构 (2.5天) ⚡ Beta版优化

| 任务ID | 任务描述 | 负责模块 | 预计耗时 | 依赖任务 | Beta版状态 |
|--------|---------|---------|---------|---------|-----------|
| T1.1 | 创建ScriptFile Prisma模型 | 数据库 | 0.5天 | - | ✅ 包含 |
| T1.2 | 编写migration脚本并执行 | 数据库 | 0.5天 | T1.1 | ✅ 包含 |
| T1.3 | 创建ScriptFileService（CRUD） | 后端 | 1天 | T1.2 | ✅ 包含 |
| T1.4 | 文件上传API实现（单个+批量） | 后端 | 1天 | T1.3 | ✅ 包含 |
| ~~T1.5~~ | ~~文件Hash检测和去重逻辑~~ | ~~后端~~ | ~~0.5天~~ | ~~T1.4~~ | ⏳ V1.1后补充 |
| T1.6 | 集数编号自动识别（正则提取） | 后端 | 0.5天 | T1.4 | ✅ 包含 |
| T1.7 | MultiFileUploader组件开发 | 前端 | 1天 | T1.4 | ✅ 包含 |
| T1.8 | 文件列表管理UI（增删改查） | 前端 | 0.5天 | T1.7 | ✅ 包含 |
| T1.9 | 单元测试：Service层 | 测试 | 0.5天 | T1.3 | ✅ 包含 |
...
### Sprint 2: JSON转换服务 (1.5天) ⚡ Beta版优化

| 任务ID | 任务描述 | 负责模块 | 预计耗时 | 依赖任务 | Beta版状态 |
|--------|---------|---------|---------|---------|-----------|
| T2.1 | 创建FastAPI项目结构 | Python微服务 | 0.5天 | - | ✅ 包含 |
| T2.2 | 复用现有Python转换代码 | Python微服务 | 0.5天 | T2.1 | ✅ 包含 |
| T2.3 | 实现/convert/script endpoint | Python微服务 | 1天 | T2.2 | ✅ 包含 |
| T2.4 | 实现/convert/outline endpoint | Python微服务 | 0.5天 | T2.3 | ✅ 包含 |
| T2.5 | DeepSeek API集成和错误处理 | Python微服务 | 0.5天 | T2.3 | ✅ 包含 |
| T2.6 | Docker镜像构建和测试 | DevOps | 0.5天 | T2.5 | ✅ 包含 |
| T2.7 | 创建ConversionService客户端 | 后端 | 0.5天 | T2.5 | ✅ 包含 |
| T2.8 | 转换API封装（Next.js） | 后端 | 0.5天 | T2.7 | ✅ 包含 |
| T2.9 | 转换状态轮询逻辑 | 后端 | 0.5天 | T2.8 | ✅ 包含 |
| ~~T2.10~~ | ~~前端转换进度展示~~ | ~~前端~~ | ~~0.5天~~ | ~~T2.9~~ | ⏳ V1.1后补充 |
| T2.11 | Docker Compose配置 | DevOps | 0.5天 | T2.6 | ✅ 包含 |

**里程碑2**: 剧本文件可自动转换为结构化JSON ✅
**节省时间**: 0.5天（进度条UI简化）
**降级方案**: 使用简单loading提示"转换中...N个文件"

---

### Sprint 3: 分层检查系统 (3天) ⚡ Beta版优化

| 任务ID | 任务描述 | 负责模块 | 预计耗时 | 依赖任务 | Beta版状态 |
|--------|---------|---------|---------|---------|-----------|
| T3.1 | 扩展DiagnosticReport结构 | 数据库 | 0.5天 | Sprint 2 | ✅ 包含 |
| T3.2 | 单文件检查：批量调用逻辑 | 后端 | 1天 | T3.1 | ✅ 包含 |
| T3.3 | 单文件检查：结果合并 | 后端 | 0.5天 | T3.2 | ✅ 包含 |
| T3.4 | 创建CrossFileAnalyzer类 | 后端 | 0.5天 | T3.3 | ✅ 包含 |
| T3.5 | 实现时间线跨文件检查 | 后端+AI | 1天 | T3.4 | ✅ 包含 |
| T3.6 | 实现角色跨文件检查 | 后端+AI | 1天 | T3.4 | ✅ 包含 |
| T3.7 | 实现情节跨文件检查 | 后端+AI | 0.5天 | T3.4 | ✅ 包含 |
| T3.8 | 实现设定跨文件检查 | 后端+AI | 0.5天 | T3.4 | ✅ 包含 |
| T3.9 | AI辅助决策Prompt设计 | AI | 1天 | T3.5-T3.8 | ✅ 包含 |
| T3.10 | 跨文件检查结果存储 | 后端 | 0.5天 | T3.9 | ✅ 包含 |
| T3.11 | 多文件分析API实现 | 后端 | 1天 | T3.10 | ✅ 包含 |
| T3.12 | 诊断报告UI重构（分组展示） | 前端 | 1天 | T3.11 | ✅ 包含 |
| ~~T3.13~~ | ~~跨文件问题关联高亮~~ | ~~前端~~ | ~~1天~~ | ~~T3.12~~ | ⏳ V1.1后补充 |
| T3.14 | 单元测试：CrossFileAnalyzer | 测试 | 0.5天 | T3.10 | ✅ 包含 |

**里程碑3**: 系统支持单文件+跨文件完整检查 ✅
**节省时间**: 1天（高亮UI简化）
**降级方案**: 使用颜色编码区分问题类型 + 清晰的文本描述关联关系
...
| Sprint | 核心任务 | 原计划 | Beta版 | 节省 | 关键交付物 |
|--------|---------|--------|--------|------|-----------|
| Sprint 1 | 多文件基础架构 | 3天 | 2.5天 | 0.5天 | ScriptFile模型、上传API、前端UI |
| Sprint 2 | JSON转换服务 | 2天 | 1.5天 | 0.5天 | Python微服务、Docker镜像、转换API |
| Sprint 3 | 分层检查系统 | 4天 | 3天 | 1天 | CrossFileAnalyzer、AI检查、简化UI |
| Sprint 4 | 整合测试与优化 | 1天 | 1天 | - | 测试报告、部署文档 |
| **总计** | **4个Sprint** | **10天** | **8天** | **2天** | **完整多文件分析系统（Beta版）** |

**削减说明**：
- ❌ T1.5（文件去重）：0.5天 → 数据库字段保留，前端提示后置
- ❌ T2.10（进度条）：0.5天 → 用简单loading替代
- ❌ T3.13（关联高亮）：1天 → 用颜色编码+文本描述替代
